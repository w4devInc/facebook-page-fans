...
(function($){
			...
			if( ! $wrap.data('listen_on_trigger_init') ){
				$(document.body).on( $wrap.data('listen_on_trigger_name'), function( e, value ){
					...
					if( 'ajax_fetch' === $wrap.data('listen_on_trigger_action') ){
						...
						var ajaxurl = $wrap.data('listen_on_ajaxurl') ? $wrap.data('listen_on_ajaxurl') : fbpf.ajaxurl;
	...
	function register_ajax_forms( $parent ){
		...
		$parent.find('.wff_ajax_form').each(function(){
			...
			if (! url) {
				if (ajaxurl) {
					url = ajaxurl;
				} else if (typeof(fbpf.ajaxUrl) !== 'undefined') {
					url = fbpf.ajaxUrl;
				}
			}
	...
	$(document).ready(function(){
		...
		$(document.body).on('fbpf/listen_on_trigger', function(e, $wrap){
			register_listen_on_trigger( $wrap );
		});
		$(document.body).on('fbpf/trigger_on_change', function(e, $wrap){
			register_trigger_on_change( $wrap );
		});
		$(document.body).on('fbpf/ajax_form', function(e, $wrap){
			register_ajax_forms( $wrap );
		});
		...
		$(document.body).on('click', '.wf_repeater_add', function(e){
			...
			$(document).trigger( 'fbpf/row_cloned' );
			return false;
		});
		...
		$(document).on('click', '.wf_repeater_remove', function(e){
			...
			$(document).trigger( 'fbpf/row_removed' );
			return false;
		});
...
(function($){
	...
	uploaderInit = function(obj){
		...
		uploader.bind('Init', function(up) {
			...
			wrapDiv.on('fbpf/start_upload', function(){
				// up.refresh();
				up.start();
			});
		});
		...
		uploader.bind('UploadComplete', function(up, files) {
			...
			wrapDiv.trigger('fbpf/upload_complete', [up, files]);
		});
	};
	...
	$(document).ready(function($){
		...
		*/
		$(document.body).on('fbpf/uploader_init', function(a, uploader){
